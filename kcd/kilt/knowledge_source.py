...
def _get_pageid_from_api(title, client=None):
    ...
    url = (
        "https://en.wikipedia.org/w/api.php?action=query&titles={}&format=json".format(title_html))
...
class KnowledgeSource:
    ...
    def get_page_by_id(self, wikipedia_id):
        page = self.db.find_one({"id": str(wikipedia_id)})
        return page

    def get_page_by_title(self, wikipedia_title, attempt=0):
        page = self.db.find_one({"id": str(wikipedia_title)})
        return page
